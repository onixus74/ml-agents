...
class Trainer(object):
    def __init__(self, model, sess, info, is_continuous, use_observations, use_states):
        """
        Responsible for collecting experinces and training PPO model.
        :param model: Tensorflow graph defining model.
        ...
        self.model = model
        self.sess = sess
        self.training_buffer = empty_local_history({})
...
class ImitationTrainer(Trainer):
    def __init__(self, model, sess, info, is_continuous, use_observations, use_states):
        Trainer.__init__(self, model, sess, info, is_continuous, use_observations, use_states)
        stats = {'episode_length': [], 'entropy': [], 'policy_loss': [], 'learning_rate': []}
        self.training_buffer = empty_local_history(self.training_buffer)
        self.stats = stats
    ...
    def update_model(self, batch_size):
        ...
        """
        train_batch = sample_buffer(self.training_buffer, batch_size)
        total_p = 0
        feed_dict = {}
        if self.is_continuous:
            feed_dict[self.model.epsilon] = np.vstack(train_batch['epsilons'])
        else:
            feed_dict[self.model.action_holder] = np.hstack(train_batch['actions'])
        if self.use_states:
            feed_dict[self.model.state_in] = np.vstack(train_batch['states'])
        if self.use_observations:
            feed_dict[self.model.observation_in] = np.vstack(train_batch['observations'])
        p_loss, _ = self.sess.run([self.model.policy_loss, self.model.update_batch], feed_dict=feed_dict)
        total_p += p_loss
        self.stats['policy_loss'].append(total_p)

    def take_action(self, player_info, agent_info, env, player_brain, agent_brain):
        """
        ...
        """
        epsi = None
        feed_dict = {self.model.batch_size: len(agent_info.states)}
        if self.is_continuous:
            epsi = np.random.randn(len(agent_info.states), env.brains[agent_brain].action_space_size)
            feed_dict[self.model.epsilon] = epsi
        if self.use_observations:
            feed_dict[self.model.observation_in] = np.vstack(agent_info.observations)
        if self.use_states:
            feed_dict[self.model.state_in] = agent_info.states
        ...
        new_infos = env.step(actions)
        new_player_info = new_infos[player_brain]
        self.add_experiences(player_info, new_player_info, epsi)
        return new_infos

    def add_experiences(self, info, new_info, epsi):
        """
        ...
        """
        epsi = np.zeros_like(epsi)
        if self.use_observations:
            self.training_buffer['observations'].append([info.observations[0][0]])
        if self.use_states:
            self.training_buffer['states'].append(info.states[0])
        if self.is_continuous:
            self.training_buffer['epsilons'].append(epsi[0])
        self.training_buffer['actions'].append(new_info.actions[0])
...
class PPOTrainer(Trainer):
    def __init__(self, model, sess, info, is_continuous, use_observations, use_states):
        ...
        self.stats = stats
        self.training_buffer = vectorize_history(self.training_buffer)
